...
class SandboxClassLoader extends URLClassLoader {
    ...
    SandboxClassLoader(final String namespace,
                       final URL[] urls) throws MalformedURLException {
        super(urls);
        this.path = "sandboxCoreJarFilePath";
        this.toString = String.format("SandboxClassLoader[namespace=%s;path=%s;]", namespace, path);
    }
...
//        for (SpiPluginLauncher.PluginJar each : jars) {
//            JarEntry entry = each.jarFile.getJarEntry(name);
//            if (Objects.nonNull(entry)) {
//                try {
//                    URL url = new URL(String.format("jar:file:%s!/%s", each.sourcePath.getAbsolutePath(), name));
...
//                    System.out.println("Failed to load class {}." + name + ":" + ex.getMessage());
//                }
//            }
//        }
